...
const destinationMobileActions = document.getElementById("destinationMobileActions");
const destinationLoadMore = document.getElementById("destinationLoadMore");
...
const MOBILE_DESTINATION_BATCH = 6;

let destinations = [];
let revealObserver = null;
let lastFocusedElement = null;
let copyToastTimeout = null;
let mobileDestinationVisibleCount = MOBILE_DESTINATION_BATCH;
let isCompactLandingViewportState = window.matchMedia("(max-width: 640px)").matches;
...
window.addEventListener("resize", () => {
  ...
  const nextCompactViewportState = isCompactLandingViewport();

  if (nextCompactViewportState !== isCompactLandingViewportState) {
    isCompactLandingViewportState = nextCompactViewportState;

    if (nextCompactViewportState) {
      resetMobileDestinationLimit();
    }
...
const isCompactLandingViewport = () => window.matchMedia("(max-width: 640px)").matches;
...
const resetMobileDestinationLimit = () => {
  mobileDestinationVisibleCount = MOBILE_DESTINATION_BATCH;
};

const updateMobileDestinationActions = (totalCount, visibleCount) => {
  if (!destinationMobileActions || !destinationLoadMore) {
    return;
  }

  const shouldShowActions = isCompactLandingViewport() && totalCount > 0;
  const hasMore = totalCount > visibleCount;

  destinationMobileActions.hidden = !shouldShowActions;
  destinationLoadMore.hidden = !hasMore;
  destinationLoadMore.disabled = !hasMore;
};
...
const renderDestinations = () => {
  ...
  const visibleDestinations = isCompactLandingViewport()
    ? filteredDestinations.slice(0, mobileDestinationVisibleCount)
    : filteredDestinations;

  destinationGrid.classList.add("is-updating");
  destinationGrid.innerHTML = visibleDestinations.map(renderDestinationCard).join("");

  destinationCount.textContent =
    isCompactLandingViewport() && filteredDestinations.length > visibleDestinations.length
      ? `${visibleDestinations.length} dari ${filteredDestinations.length} destinasi ditampilkan di mobile`
      : `${filteredDestinations.length} dari ${destinations.length} destinasi tampil`;
  ...
  updateMobileDestinationActions(filteredDestinations.length, visibleDestinations.length);
...
const showDestinationError = () => {
  ...
  if (destinationMobileActions) {
    destinationMobileActions.hidden = true;
  }
};
...
[destinationSearch, categoryFilter, regionFilter].forEach((control) => {
  control?.addEventListener("input", handleDestinationFilterChange);
  control?.addEventListener("change", handleDestinationFilterChange);
});
...
destinationLoadMore?.addEventListener("click", () => {
  mobileDestinationVisibleCount += MOBILE_DESTINATION_BATCH;
  renderDestinations();
});
